...
int smithyCard(int currentPlayer, struct gameState *state, int handPos)
{
	...
	return discardCard(handPos, currentPlayer, state, 1);//added return of whatever discard card returns.
}
...
void adventurerCard(int currentPlayer, int temphand[], int z, struct gameState *state)
{
	int cardDrawn;
	int drawntreasure = 0;
	while (drawntreasure<=2) {//changed "<" to "<=" to create a bug.
...
int discardCard(int handPos, int currentPlayer, struct gameState *state, int trashFlag)
{
  ...
  if ( handPos == (state->handCount[currentPlayer] - 1) ) 	//last card in hand array is played
  ...
  else 	
    {
      ...
      //reduce number of cards in hand
		if(state->handCount[currentPlayer] == 0){//created to prevent a player from discarding more cards than he has.
			return -1;
		}
      state->handCount[currentPlayer]--;
    }
